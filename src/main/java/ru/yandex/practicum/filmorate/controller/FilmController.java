...
public class FilmController {
    ...
    public Film addNewFilm(@RequestBody @Valid Film film) {
        log.info("PF-1. Received GET request: new film");
        return filmStorage.add(film);
    }

    @PutMapping
    public Film updateFilm(@RequestBody @Valid Film film) {
        log.info("PF-2. Received PUT request: update film {}", film.getId());
        return filmStorage.update(film.getId(), film);
    }

    @GetMapping
    public Set<Film> findAllFilms() {
        log.info("GF-1. Received GET request: all films");
        return filmStorage.getAll();
    }

    @PutMapping("/{id}/like/{userId}")
    public Film addLike(@PathVariable("id") Integer id, @PathVariable("userId") Integer userId) {
        log.info("PF-3. Received PUT request: add like to film {} from user {}", id, userId);
        return filmService.addLike(id, userId);
    }

    @DeleteMapping("/{id}/like/{userId}")
    public Film deleteLike(@PathVariable("id") Integer id, @PathVariable("userId") Integer userId) {
        log.info("DF-2 .Received DELETE request: remove like of film {} form user {}", id, userId);
        return filmService.deleteLike(id, userId);
    }

    @GetMapping("/{id}")
    public Film getFilmById(@PathVariable("id") Integer id) {
        log.info("GF-2. Received GET request: film {}", id);
        return filmService.get(id);
    }

    @GetMapping("/common")
    public List<Film> getCommonFilms(@RequestParam() Integer userId, @RequestParam() Integer friendId) {
        log.info("GF-3. Received GET request: common films of users {} and {}", userId, friendId);
        return filmService.getCommonFilms(userId, friendId);
    }

    @GetMapping("/director/{id}")
    public List<Film> getSortedFilmsWithIdDirector(@PathVariable("id") Integer id, @RequestParam String sortBy) {
        log.info("GF-4. Received GET request: films of director {} sorted by {}", id, sortBy);
    ...
    public List<Film> getFilmsByKeyWord(@RequestParam String query, @RequestParam String by) {
        log.info("GF-6. Received GET request: films by {} sorted by {}", query, by);
    ...
    public List<Film> getMostPopularFilms(@RequestParam(defaultValue = "10") Integer count,
        ...
        log.info("GF-5. Received GET request: top {} popular films by year and genre", limit);
        return filmService.getMostPopularFilms(count, limit, genreId, year);
    }

    @DeleteMapping("/{id}")
    public Set<Film> delete(@PathVariable("id") Integer id) {
        log.info("DF-1 .Received DELETE request: film {}", id);
